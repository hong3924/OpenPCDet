...
bash scripts/dist_train.sh ${NUM_GPUS} --cfg_file ${CONFIG_FILE} --pretrained_model ${CKPT_FILE}

# Training with single GPU
python train.py --cfg_file ${CONFIG_FILE} --pretrained_model ${CKPT_FILE}

# Testing with multiple GPUs
bash scripts/dist_test.sh ${NUM_GPUS} --cfg_file ${CONFIG_FILE}--ckpt_dir ${OUTPUT_PATH} --eval_all

# Testing with single GPU
python test.py --cfg_file ${CONFIG_FILE} --ckpt_dir ${OUTPUT_PATH} --eval_all
```
- NUM_GPUS: The number of GPUs that you can use. `CUDA_VISIBLE_DEVISES` might be helpful.
- CONFIG_FILE: Default is `../dsvt_pillar.yaml`
- CKPT_FILE: Default is `../dsvt-p.pth`
- OUTPUT_PATH: Default is `../output/kitti_models/dsvt_pillar/default/ckpt/`
