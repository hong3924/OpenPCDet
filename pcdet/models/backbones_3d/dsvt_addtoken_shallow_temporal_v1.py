# dsvt_addtoken_shallow_temporal_v1.py
import torch
...
class SetAttention(nn.Module):
    ...
    def incorporate_prompt(self, query, key, value, pi):
        ...
        prompt_query = torch.cat((self.prompt_proj(pi.expand(set_num, -1, -1)), query), dim=1)
        prompt_key = torch.cat((self.prompt_proj(pi.expand(set_num, -1, -1)), key), dim=1) 
        prompt_value = torch.cat((self.prompt_proj(pi.expand(set_num, -1, -1)), value), dim=1)
    ...
    def forward(self, src, shallow, pos=None, key_padding_mask=None, voxel_inds=None):
        ...
        # prompt gennerator
        if shallow:
            pi = self.PromptGenerator(set_features)
            # pi = torch.rand(self.num_prompt, self.dim)
            # nn.init.xavier_uniform_(pi)
            # pi = pi.to(self.prompt.device)
            # print(f'pi: ', pi)
            # cur_prompt = torch.add(self.prompt, pi)
...
class PromptGenerator(nn.Module):
    ...
    def __init__(self, num_prompt, dim, d_model, nhead, dropout, batch_first=True):
        ...
        self.dim = dim
        self.memory_prompt = nn.Parameter(torch.zeros(self.num_prompt, self.dim))
        # self.memory_prompt = torch.randn(1, self.num_prompt, self.dim)
        self.prompt_proj = nn.Identity()
    ...
    def exclude_prompt(self, temp_prompt):
        ...
        '''
        split_prompt, _ = torch.split(temp_prompt, [temp_prompt.size(1)-36, 36], dim=1)
        mean_prompt = torch.mean(split_prompt, dim=0, keepdim=True)
        new_mem_prompt = mean_prompt.squeeze(0)
        # print(f'mean_prompt: ', mean_prompt.size())
        # print(f'new_mem_prompt: ', new_mem_prompt.size())
        return new_mem_prompt

    def forward(self, scene):
        temp_prompt = self.incorporate_prompt(scene)
        query = key = value = temp_prompt
        temp_prompt = self.self_attn(query, key, value)[0]
        new_mem_prompt = self.exclude_prompt(temp_prompt)
        self.memory_prompt.data = new_mem_prompt

        return self.memory_prompt.data
