# dsvt_addtoken_shallow_temporal_v4.py
import torch
...
class DSVT(nn.Module):
    ...
    def __init__(self, model_cfg, **kwargs):
        ...
        block_name = self.model_cfg.block_name # 'DSVTBlock'
        set_info = self.model_cfg.set_info # [[36, 4]]
        d_model = self.model_cfg.d_model # 192
        nhead = self.model_cfg.nhead # 8
        dim_feedforward = self.model_cfg.dim_feedforward # 384
        dropout = self.model_cfg.dropout # 0.0
        activation = self.model_cfg.activation # gelu
        self.reduction_type = self.model_cfg.get('reduction_type', 'attention') # 'attention'(沒有'reduction_type'的話，設為'attention')
        # save GPU memory
        ...
        for stage_id in range(stage_num):
            num_blocks_this_stage = set_info[stage_id][-1] # 4
            dmodel_this_stage = d_model[stage_id] # 192
            dfeed_this_stage = dim_feedforward[stage_id] # 384
            num_head_this_stage = nhead[stage_id] # 8
            block_name_this_stage = block_name[stage_id] # 'DSVTBlock'
            block_module = _get_block_module(block_name_this_stage) # DSVTBlock
            block_list=[]
            norm_list=[]
            for i in range(num_blocks_this_stage):
                ...
                # block_list = [DSVTBlock, DSVTBlock, DSVTBlock, DSVTBlock]
                norm_list.append(nn.LayerNorm(dmodel_this_stage))
                # norm_list = [LayerNorm(192), LayerNorm(192), LayerNorm(192), LayerNorm(192)]
            self.__setattr__(f'stage_{stage_id}', nn.ModuleList(block_list)) 
            # stage_0 = nn.ModuleList(block_list[DSVTBlock, DSVTBlock, DSVTBlock, DSVTBlock])
            self.__setattr__(f'residual_norm_stage_{stage_id}', nn.ModuleList(norm_list))
            # residual_norm_stage_0 = nn.ModuleList(norm_list[LayerNorm(192), LayerNorm(192), LayerNorm(192), LayerNorm(192)])

            # apply pooling except the last stage
            if stage_id < stage_num-1:
                downsample_window = self.model_cfg.INPUT_LAYER.downsample_stride[stage_id] # [[1, 1, 4], [1, 1, 4], [1, 1, 2]]
                ...
                if self.reduction_type == 'linear':
                    ...
                    self.__setattr__(f'stage_{stage_id}_reduction', Stage_ReductionAtt_Block(dmodel_this_stage, pool_volume)) # 192, 4
                else:
                    raise NotImplementedError

        self.num_shifts = [2] * stage_num # [2]
        self.output_shape = self.model_cfg.output_shape # [468, 468]
        self.stage_num = stage_num # 1
        self.set_info = set_info # [[36, 4]]
        self.num_point_features = self.model_cfg.conv_out_channel # 192
...
class SetAttention(nn.Module):
    ...
    def __init__(self, d_model, nhead, dropout, dim_feedforward=2048, activation="relu", batch_first=True, mlp_dropout=0):
        ...
        # prompt generator
        self.PromptGenerator = PromptGenerator(self.d_model)


    def incorporate_prompt(self, query, key, value, scene_prompt):
        '''
        Args:
            prompt(num_prompt, C)
            scene_prompt(set_num, num_prompt, C)
        ...
        temp_prompt = torch.add(scene_prompt, (self.prompt_proj((self.prompt).expand(set_num, -1, -1))))
        prompt_query = torch.cat((self.prompt_proj(temp_prompt), query), dim=1)
        prompt_key = torch.cat((self.prompt_proj(temp_prompt), key), dim=1) 
        prompt_value = torch.cat((self.prompt_proj(temp_prompt), value), dim=1)
    ...
    def forward(self, src, shallow, pos=None, key_padding_mask=None, voxel_inds=None):
        ...
        Args:
            ...
            voxel_inds (Tensor[int]): Voxel indexs for each set. Shape of (set_num, set_size). (?, 36)
        Returns:
        ...
        if pos is not None:
            ...
            set_pos = None
        if pos is not None:
            query = set_features + set_pos # (set_num, set_size=36, C=192)
            key = set_features + set_pos   # (set_num, set_size=36, C=192)
            value = set_features           # (set_num, set_size=36, C=192)

        # prompt gennerator
        if shallow:
            scene_prompt = self.PromptGenerator(set_features)
            query, key, value = self.incorporate_prompt(query, key, value, scene_prompt)
...
class PromptGenerator(nn.Module):
    ...
    def __init__(self, d_model, dim_feedforward=2048, activation="relu"):
        ...
        self.norm1 = nn.BatchNorm1d(36) # nn.LayerNorm(d_model)
    ...
    def forward(self, set_features):
        ...
        set_features = set_features.clone() # (set_num, 36, 192)
        set_features = self.norm1(set_features) # (set_num, 36, 192)
        set_features = self.activation(self.norm1(self.linear1(set_features))) # (set_num, 36, 2048)
        set_features = self.activation(self.norm1(self.linear2(set_features))) # (set_num, 36, 2048)
        set_features = self.activation(self.norm1(self.linear3(set_features))) # (set_num, 36, 2048)
        set_features = self.activation(self.norm1(self.linear4(set_features))) # (set_num, 36, 192)
        set_features = torch.mean(set_features, dim=1, keepdim=True) # (set_num, 1, 192)
        return set_features
...
class Stage_ReductionAtt_Block(nn.Module): 
    def __init__(self, input_channel, pool_volume): # 192, 4
        super().__init__()
    ...
    def forward(self, x, key_padding_mask):  # prepool_features, key_padding_mask
        # x: [voxel_num, c_dim, pool_volume]
        src = self.query_func(x).permute(0, 2, 1)  # voxel_num, 1, c_dim
        key = value = x.permute(0, 2, 1) # voxel_num, pool_volume, c_dim
...
class DSVTInputLayer(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.model_cfg = model_cfg 
        self.sparse_shape = self.model_cfg.sparse_shape # [468, 468, 1]
        self.window_shape = self.model_cfg.window_shape # [[12, 12, 1]]
        self.downsample_stride = self.model_cfg.downsample_stride # []
        self.d_model = self.model_cfg.d_model # [192]
        self.set_info = self.model_cfg.set_info # [[36, 4]]
        self.stage_num = len(self.d_model) # 1

        self.hybrid_factor = self.model_cfg.hybrid_factor # [2, 2, 1]
        self.window_shape = [[self.window_shape[s_id], [self.window_shape[s_id][coord_id] * self.hybrid_factor[coord_id] \
                                                for coord_id in range(3)]] for s_id in range(self.stage_num)] # [[12, 12, 1], [24, 24, 1]]
        self.shift_list = self.model_cfg.shifts_list # [[[0, 0, 0], [6, 6, 0]]]
        self.normalize_pos = self.model_cfg.normalize_pos # False
            
        self.num_shifts = [2,] * len(self.window_shape) # [2, 2]

        self.sparse_shape_list = [self.sparse_shape] # [[468, 468, 1]]
        ...
        for i in range(len(self.set_info)):
            input_dim = 3 if self.sparse_shape_list[i][-1] > 1 else 2 # sparse_shape_list[i][-1]=1
            stage_posembed_layers = nn.ModuleList()
            for j in range(self.set_info[i][1]): # set_info[i][1]=4
                block_posembed_layers = nn.ModuleList()
                for s in range(self.num_shifts[i]): # num_shifts[i]=2
