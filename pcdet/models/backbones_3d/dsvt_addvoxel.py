...
class DSVT(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        # set_voxel_masks_size_list = [[voxel_info[f'set_voxel_mask_stage{s}_shift{i}'].size() for i in range(self.num_shifts[s])] for s in range(self.stage_num)]
        
        # print(f'set_voxel_inds_list[0][0]:', set_voxel_inds_list[0][0])
        # len_set_voxel_inds_list = [len(a) for a in set_voxel_inds_list[0][0]]
        # print(len_set_voxel_inds_list)
        ...
        for stage_id in range(self.stage_num): # stage_num = 1
            ...
            for i in range(len(block_layers)): # block_layers, 重複4次的 DSVTBlock
...
class DSVTBlock(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        encoder_1 = DSVT_EncoderLayer(d_model, nhead, dim_feedforward, dropout, # x 方向 partition
                                        activation, batch_first)
        encoder_2 = DSVT_EncoderLayer(d_model, nhead, dim_feedforward, dropout, # y 方向 partition
                                        activation, batch_first)
    ...
    def forward(
        ...
        for i in range(num_shifts): # x, y 兩方向 partition
            set_id = i
            shift_id = block_id % 2 
            pos_embed_id = i
...
class SetAttention(nn.Module):
    ...
    def forward(self, src, pos=None, key_padding_mask=None, voxel_inds=None):
        ...
        print(f'voxel_inds:', voxel_inds.size())
        # print(f'set_features:', set_features.size())
        print(f'src2:', src2.size())
...
class DSVTInputLayer(nn.Module):
    ...
    # def incorporate_prompt(self, voxel_feats):
    #     prompt_voxel_feats = torch.cat((self.prompt, voxel_feats), dim=0)
    #     return prompt_voxel_feats

    # def incorporate_coors(self, voxel_coors):
    #     prompt_coors = torch.zeros(self.num_prompt, 4, device=voxel_coors.device)
    #     prompt_voxel_coors = torch.cat((prompt_coors, voxel_coors), dim=0)
    #     return prompt_voxel_coors
    ...
    def forward(self, batch_dict):
        ...
        # prompt
        # voxel_info['voxel_feats_stage0'] = self.incorporate_prompt(voxel_feats.clone())
        # voxel_info['voxel_coors_stage0'] = self.incorporate_coors(voxel_coors.clone()).long()
        # print(f'prompt_voxel_feats', voxel_info['voxel_feats_stage0'].shape)
        # print(f'prompt_voxel_coors', voxel_info['voxel_coors_stage0'].shape)
