...
class prompt_generator(nn.Module):
    ...
    def forward(self, src):
        ...
        pi = self.norm2(self.activation(self.linear1(pi))) # (N, 2048)
        pi = self.norm2(self.activation(self.linear2(pi))) # (N, 2048)
        pi = self.norm2(self.activation(self.linear3(pi))) # (N, 2048)
        pi = self.norm1(self.activation(self.linear4(pi))) # (N, 192)
...
class DSVTInputLayer(nn.Module):
    ...
    def __init__(self, model_cfg):
        ...
        self.prompt_generator = nn.ModuleList([prompt_generator(self.d_model[0])]) # 將 prompt_generator 加入 ModuleList
        
    def incorporate_prompt(self, voxel_feats, pi):
        self.prompt.data += pi.expand(100, -1) # add dynamic pi to prompt
        prompt_voxel_feats = torch.cat((self.prompt, voxel_feats), dim=0) # concatenate to voxel (N, C) -> (N + 100, C)
    ...
    def forward(self, batch_dict):
        ...
        # prompt generator
        pi = self.prompt_generator[0](batch_dict['voxel_features']) # self.prompt_generator 此 nn.ModuleList 中的第一個模塊
        # prompt
        voxel_info['voxel_feats_stage0'] = self.incorporate_prompt(voxel_feats.clone(), pi)
